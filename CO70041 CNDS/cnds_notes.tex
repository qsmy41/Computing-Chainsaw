...
\begin{itemize}
    ...
    \item screened host (dual-homed) architecture
        ...
        \end{itemize} 
        \begin{figure}[h]
          	\includegraphics[scale=0.25]{dual_homed.png}
          	\centering
        \end{figure}
    \item screened subnet architecture
        \begin{itemize}
            \item pass packets for which the source of destination IP
                address is the bastion host
            \item pass packets for which the ports are for defined proxies on
                the gateway
